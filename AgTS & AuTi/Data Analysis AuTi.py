...
shortcounter=0
devcount=0
devs=np.linspace(0,8,9)
counter2=np.linspace(0,8,9)
stdavg=np.linspace(0,8,9)
scounts=np.linspace(0,8,9)
shorting=np.linspace(0,8,9)
...
hums=[58.94625,49.4325,55.9125,73.15]
std_hum = [15.61982829,19.53188483,22.90592541,17.41114839]
...
shorting = [10,22,20,18,18,28,50,22,20]
'''
shorting[1] = 22
shorting[2] = 20
shorting[3] = 18
...
hums=[33.71,35.22,27.55,47.92,79.6,57.36,72.36,45.14,78.29]
durationTime = ['1','2','3','4','5','>=6']
duration_time = [-100] * 6

duration_time[0] = counter2[0]
duration_time[0] += counter2[1]
...
plt.xticks(np.arange(9, 17 + 1, 1),['C9','C10','C11','C12','C13','C14','C15','C16','C17'],fontsize = 15)
...
plt.text(-0.1,22,'n =' + str(spots_with_values_greater_than_3_0),fontsize=40)
plt.text(0.8,7,'n =' + str(spots_with_values_greater_than_3_1),fontsize=40)
#plt.text(0.3,8,'No Measurement Taken in Autumn and Winter',fontsize=40,color='red')
...
#label = ['6AM - 10AM','10AM - 2PM','2PM - 6PM','6PM - 10PM']
...
#print(avg)
'''
#hums=[73.3549,58.94625,49.4325,55.9125] #INCLUDE 10PM-10AM
#hums=[73.7628,58.94625,49.4325,55.9125] #INCLUDE 6AM-10AM
hums=[58.94625,49.4325,55.9125,73.15]
...
ax2.set_ylabel('Humidity(%)', color='blue',fontsize=40)
ax2.tick_params(axis='y', labelcolor='blue',labelsize=40)
ax1.text(0,26,'n =' + str(total_spots_time[0]),fontsize=40)
ax1.text(0.75,7,'n =' + str(total_spots_time[1]),fontsize=40)
ax1.text(1.75,13,'n =' + str(total_spots_time[2]),fontsize=40)
ax1.text(2.75,20,'n =' + str(total_spots_time[3]),fontsize=40)
ax1.set_ylim(-3,50)
for x in range(len(timePeriod)):
    if not(std_time[x] == -1):
        ax1.errorbar(label[x], counter2tot[x],yerr=std_time[x],fmt='none',capsize=20,ecolor='red')
