...
fc='fc'
c=chr(92)
carbo_folder_path= r'F:\Thuo Research Group\MAA'
#carbonLen = [9,11,13,15]
dogs=np.linspace(11,13,3)
...
shortcounter=0
devcount=0
devs=np.linspace(11,13,3)
counter2=np.linspace(11,13,3)
stdavg=np.linspace(11,13,3)
scounts=np.linspace(11,13,3)
shorting=np.linspace(11,13,3)
    ...
    for scfc_line, spot_data in morning.items():
        for spot_number, file_paths in spot_data.items():
            ...
            if any(np.log10(concatenated_data['Absolute Value of J']) > 3):
                ...
                est_morning.append(1/len(file_paths))
            else:
                est_morning.append(0)
...
std_hum = [10.73889074,15.65543081,8.178465754,13.86949318]
...
ax1.text(0,5,'n =' + str(spots_with_values_greater_than_3_1),fontsize=40)
ax1.text(0.75,5,'n =' + str(spots_with_values_greater_than_3_2),fontsize=40)
ax1.text(1.75,5,'n =' + str(spots_with_values_greater_than_3_3),fontsize=40)
ax1.text(2.75,15,'n =' + str(spots_with_values_greater_than_3_4),fontsize=40)
ax1.set_ylim(-3,50)
...
carbonNum = ['C11NHBu','C12NHMe 3h','C12NHMe RT']
hums=[83.205,73.859,67.69]


plt.scatter(dogs,counter2,c='black',s=80)
plt.xticks(np.arange(11, 13 + 1, 1),['C11NHBu','C12NHMe 3h','C12NHMe RT'],fontsize = 15)
...
label = ['6AM - 10AM','10AM - 2PM','6PM - 10PM','10PM - 6AM']
...
ax2.set_ylabel('Humidity(%)', color='blue',fontsize=40)
ax2.tick_params(axis='y', labelcolor='blue',labelsize=40)
ax1.text(-0.1,2,'n =' + str(total_spots_time[0]),fontsize=40)
ax1.text(0.8,2,'n =' + str(total_spots_time[1]),fontsize=40)
ax1.text(1.8,2,'n =' + str(total_spots_time[2]),fontsize=40)
ax1.text(2.6,5,'n =' + str(total_spots_time[3]),fontsize=40)
ax1.set_ylim(-3,50)
for x in range(len(timePeriod)):
    if not(std_time[x] == -1):
        ax1.errorbar(label[x], counter2tot[x],yerr=std_time[x],fmt='none',capsize=20,ecolor='red')
