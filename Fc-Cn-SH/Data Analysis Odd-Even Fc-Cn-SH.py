...
fc='fc'
c=chr(92)
carbo_folder_path= r'Q:\Shared drives\MSE-Thuo Group\Group Server ISU\Zizhi\Data\Data Analysis\Origin Data\Fc-Cn-SH'
#carbonLen = [9,11,13,15]
dogs=np.linspace(6,15,10)
...
shortcounter=0
devcount=0
devs=np.linspace(6,15,10)
counter2=np.linspace(6,15,10)
stdavg=np.linspace(6,15,10)
scounts=np.linspace(6,15,10)
shorting=np.linspace(6,15,10)
...
def group_files_by_scfc_and_spot(file_paths):
    ...
    pattern = r"sc(\d+)fc\\spot (\d+)" # Change if dataset format changes - current (sc15fc/spot 1)
    grouped_files = {}
...
hums=[69.01104167,63.75895833,75.42541667,85.77347368]
std_hum = [5.650679988,5.987263997,7.924359059,4.944721325]
...
ax1.text(2.75,-2.5,'n =' + str(spots_with_values_greater_than_3_4),fontsize=40)
ax1.set_ylim(-3,55)
...
carbonNum = ['C6','C7','C8','C9','C10','C11','C12','C13','C14','C15']
hums=[84.71,71.51333,71.001,69.3225,0,67.36538,70.257,72.30867,70.68688,73.19765]


plt.scatter(dogs,counter2,c='black',s=80)
plt.xticks(np.arange(6, 15 + 1, 1),['C6','C7','C8','C9','C10','C11','C12','C13','C14','C15'],fontsize = 15)
...
ax1.set_xlabel('Carbon Length', fontweight='bold', labelpad=15, fontsize=40)
ax1.set_ylabel('Denstiy of Junctions with STD>{}'.format(round(std,3)), color='red',fontsize=40)
# Plot the data points and the fitted curve for counter2tot
...
plt.text(0.8,45,'n =' + str(spots_with_values_greater_than_3_1),fontsize=40)
#plt.text(2.8,12,'n =' + str(total_spots_seasons[3]),fontsize=40)
for x in range(len(timePeriod)):
...
'''
hums=[69.01104167,63.75895833,75.42541667,85.77347368]   #10pm-6am
...
ax2.set_ylabel('Humidity(%)', color='blue',fontsize=40)
ax2.tick_params(axis='y', labelcolor='blue',labelsize=40)
ax1.text(-0.1,8,'n =' + str(total_spots_time[0]),fontsize=40)
ax1.text(0.8,23,'n =' + str(total_spots_time[1]),fontsize=40)
ax1.text(1.8,24,'n =' + str(total_spots_time[2]),fontsize=40)
ax1.text(2.8,14,'n =' + str(total_spots_time[3]),fontsize=40)
ax1.set_ylim(-3,55)
for x in range(len(timePeriod)):
    if not(std_time[x] == -1):
        ax1.errorbar(label[x], counter2tot[x],yerr=std_time[x],fmt='none',capsize=20,ecolor='red')
