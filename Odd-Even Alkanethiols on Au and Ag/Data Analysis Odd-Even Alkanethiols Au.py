# -*- coding: utf-8 -*-
"""
Created on Wed Sep 20 16:33:33 2023
...
fc='fc'
c=chr(92)
carbo_folder_path= r'F:\Thuo Research Group\RE Martin data\odd-even alkanethiols on Au'
#carbonLen = [9,11,13,15]
dogs=np.linspace(9,16,8)
...
hums=[70.20533333, 68.4425, 72.86854167,77.62825]    
label = ['Noon','Afternoon','Evening','Night']
std_hum = [21.98543801,23.52616088,17,12]
...
ax1.text(1.5,-2,'n =' + str(spots_with_values_greater_than_3_3),fontsize=40)
...
#ax2.fill_between(np.array(label),np.array(hums) + np.array(std_hum),np.array(hums) - np.array(std_hum),alpha=0.2)
ax2.set_ylim(0,100)
plt.tight_layout()
plt.show()    
...
carbonNum = ['C9','C10','C11','C12','C13','C14','C15','C16']
hums=[60.68,94.76,61.0925,87.74,30.925,86.26167,54.3375,77.777]
durationTime = ['1','2','3','4','5','>=6']
duration_time = [0] * 6

duration_time[1] += counter2[0]
duration_time[3] += counter2[1]
duration_time[3] += counter2[2]
duration_time[2] += counter2[3]
duration_time[1] += counter2[4]
duration_time[3] += counter2[5]
duration_time[2] += counter2[6]
duration_time[1] += counter2[7]

duration_time[1] = duration_time[1]/48 * 100
duration_time[2] = duration_time[2]/40 * 100
duration_time[3] = duration_time[3]/71 * 100
for x in range(len(duration_time)):
    if(duration_time[x] == 0):
        duration_time[x] = -100

plt.scatter(dogs,counter2,c='black',s=80)
plt.xticks(np.arange(9, 16 + 1, 1),['C9','C10','C11','C12','C13','C14','C15','C16'],fontsize = 15)
...
for index in range(len(ratios)):
    if(int(carbonNum[index][1:]) % 2) == 0:
        ax1.scatter(carbonNum[index], ratios[index], color='white',s=80, marker="s", edgecolors="red")
    else:
        ax1.scatter(carbonNum[index], ratios[index], color='red',s=80, marker="s")
#ax1.scatter(carbonNum, ratios, color='red',s=80)
ax1.tick_params(axis='x', labelsize = 14)
ax1.tick_params(axis='y', labelcolor='red',labelsize = 15)

ax2 = ax1.twinx()
ax2.scatter(carbonNum, hums, color='blue',s=80,marker = 's')
...
for index in range(len(densitystd)):
    if(int(carbonNum[index][1:]) % 2) == 0:
        ax1.scatter(carbonNum[index], densitystd[index], color='white',s=80, marker="s", edgecolors="red")
    else:
        ax1.scatter(carbonNum[index], densitystd[index], color='red',s=80, marker="s")
...
plt.text(1.8,6,'n =' + str(spots_with_values_greater_than_3_2),fontsize=40)
plt.text(2.8,12,'n =' + str(spots_with_values_greater_than_3_3),fontsize=40)
for x in range(len(timePeriod)):
...
'''
hums=[70.20533333, 68.4425, 72.86854167,77.62825]   #10pm-6am
...
ax2.set_ylabel('Humidity(%)', color='blue',fontsize=40)
ax2.tick_params(axis='y', labelcolor='blue',labelsize=40)
ax1.text(-0.25,4,'n =' + str(total_spots_time[0]),fontsize=40)
ax1.text(0.8,4,'n =' + str(total_spots_time[1]),fontsize=40)
ax1.text(1.8,2,'n =' + str(total_spots_time[2]),fontsize=40)
ax1.text(2.8,1,'n =' + str(total_spots_time[3]),fontsize=40)
ax1.set_ylim(-3,50)
for x in range(len(timePeriod)):
    if not(std_time[x] == -1):
        ax1.errorbar(label[x], counter2tot[x],yerr=std_time[x],fmt='none',capsize=20,ecolor='red')
