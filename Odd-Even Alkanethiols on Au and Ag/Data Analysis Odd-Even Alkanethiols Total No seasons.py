...
For Au
'''
devs2=np.linspace(0,7,8)
counter22=np.linspace(0,7,8)
stdavg2=np.linspace(0,7,8)
scounts2=np.linspace(0,7,8)
shorting2=np.linspace(0,7,8)
...
carbonNum = ['C7','C8','C9','C10','C11','C12','C13','C14','C15','C16','C17','C18','C19']
...
hums2=[60.68,94.76,61.0925,87.74,30.925,86.26167,54.3375,77.777]
...
for c in range(len(carbonNum)):    
    if(c < 2):
        if(int(carbonNum[c][1:]) % 2) == 0:
            ...
            ax1.scatter(carbonNum[c], ratios[c1], color='red',s=500, marker="o",label='Odd')
        c1 += 1
    elif(c >= 2 and c < 10):
        if(c == 3 or c == 5):
            if(int(carbonNum[c][1:]) % 2) == 0:
                ax1.scatter(carbonNum[c], ratios2[c2], color='white',s=500, marker="s", edgecolors="red")
            else:
...
ax2.set_ylim(20,100)
pointAgOdd = Line2D([0], [0], label='Ag Odd', color='red',marker='o',linestyle='')
pointAgEven = Line2D([0], [0], label='Ag Even', color='white',markeredgecolor="red",marker='o',linestyle='')
pointAuOdd = Line2D([0], [0], label='Au Odd', color='red',marker='s',linestyle='')
pointAuEven = Line2D([0], [0], label='Au Even', color='white',markeredgecolor="red",marker='s',linestyle='')
pointAghum = Line2D([0], [0], label='Ag hum%', color='blue',marker='o',linestyle='')
pointAuhum = Line2D([0], [0], label='Au hum%', color='blue',marker='s',linestyle='')
...
plt.yticks(np.arange(0,max(counter2tot)+2,2),fontsize = 40)
plt.grid(True,alpha=0.15)
...
label = ['10PM - 10AM','10AM - 2PM','2PM - 6PM','6PM - 10PM']
plt.xticks(np.arange(0,4,1),label,fontsize = 40)
plt.yticks(np.arange(0,20,2),fontsize = 40)
plt.grid(True,alpha=0.15)
plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter())
pointAg = Line2D([0], [0], label='Ag', color='black',marker='o',linestyle='')
pointAu = Line2D([0], [0], label='Au', color='black',marker='s',linestyle='')
